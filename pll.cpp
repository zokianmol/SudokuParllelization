...
struct tree{
	...
	void temp_gen_dist(){
		...
		for(int r=0;r<N;r++){
			for(int c=0;c<N;c++){
				temp_grid[r][c]=t[size-1].grid[r][c];
			}
		}
		int i=1;
		for(int depth=0;depth<N;depth++){
			if(t[size-1].po[row[size-1]][col[size-1]][depth]==1){
				temp_grid[row[size-1]][col[size-1]]=depth+1;
				MPI_Send(temp_grid, sizeof(temp_grid), MPI_INT, i, 0, MPI_COMM_WORLD);     //Tag=0 :		
				printf("\nData sent. to %d\n",i);
				i++;	
			}
		}
	...
	void temp_receive(){
		int temp_grid[N][N];
		bool temp_po[N][N][N];
		MPI_Recv(temp_grid,sizeof(temp_grid), MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);//MPI_STATUS_IGNORE : ignore data receive completion
		printf("\nData received.\n");
		tree_point tp;
		init(temp_grid,temp_po);
		tp.copy(temp_grid,temp_po);
		push(tp);
	}
	void print_info(){
...
else if(id==1)	// Process with id exactly equal to 1
{
		//Initialize receive array to some other data
		//t.print_info();
		t.temp_receive();
		t.pg();
		printf("success from %d\n",id);
		do{
		}while(t.forward_march() && status==0);
		printf("success from %d\n",id);
		status=1;
		MPI_Bcast(&status, 1, MPI_INT, 1, MPI_COMM_WORLD);   //arg: (data,number of data,data type,from which station,group) 
	
}	

else if(id==2)	// Process with id exactly equal to 1
{
		//Initialize receive array to some other data
		//t.print_info();
		t.temp_receive();
		...
		printf("success from %d\n",id);
		status=1;
		MPI_Bcast(&status, 1, MPI_INT, 2, MPI_COMM_WORLD);   //arg: (data,number of data,data type,from which station,group) 
		
}
else if(id==3)	// Process with id exactly equal to 1
{
		//Initialize receive array to some other data
		//t.print_info();
		t.temp_receive();
		...
		t.pg();
		printf("success from %d\n",status);
		MPI_Bcast(&status, 1, MPI_INT, 3, MPI_COMM_WORLD);   //arg: (data,number of data,data type,from which station,group) 
		printf("bc success from %d\n",id);
		
}
else{
printf("unsuccess from ");	
}
